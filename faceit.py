...
class FaceIt:
    ...
    def all_videos(self):
        return self._people[self._person_a]['videos'] + \
            self._people[self._person_b]['videos']
    ...
    def _extract_frames(self, person, video):
        ...
        print(
            '[extract-frames] about to extract_frames for {}, fps {}, length {}s'.format(
                video_frames_dir,
                video_clip.fps,
                video_clip.duration))
        ...
        if os.path.exists(video_frames_dir):
            print(
                '[extract-frames] frames already exist, skipping extraction: {}'.format(video_frames_dir))
            return
        ...
        for frame in tqdm.tqdm(
                video_clip.iter_frames(
                    fps=video['fps']),
                total=video_clip.fps *
                video_clip.duration):
            video_frame_file = os.path.join(
                video_frames_dir, 'frame_{:03d}.jpg'.format(frame_num))
        ...
        print(
            '[extract] finished extract_frames for {}, total frames {}, time taken {:.0f}s'.format(
                video_frames_dir,
                frame_num -
                1,
                time.time() -
    ...
    def _extract_faces(self, person, video):
        ...
        if os.path.exists(video_faces_dir):
            print(
                '[extract-faces] faces already exist, skipping face extraction: {}'.format(video_faces_dir))
            return

        os.makedirs(video_faces_dir)
        self._faceswap.extract(
            self._video_frames_path(video),
            video_faces_dir,
            self._people[person]['faces']
        )
    ...
    def _extract_faces_from_photos(self, person, photo_dir):
        ...
        if os.path.exists(photo_faces_dir):
            print(
                '[extract-faces] faces already exist, skipping face extraction: {}'.format(photo_faces_dir))
            return

        os.makedirs(photo_faces_dir)
        self._faceswap.extract(
            self._video_path({'name': photo_dir}),
            photo_faces_dir, self._people[person]['faces']
        )
    ...
    def _symlink_faces_for_model(self, person, video):
        ...
        for face_file in os.listdir(self._video_faces_path(video)):
            target_file = os.path.join(
                self._model_person_data_path(person),
                video['name'] + "_" + face_file)
            face_file_path = os.path.join(
                os.getcwd(), self._video_faces_path(video), face_file)
    ...
    def train(self, use_gan=False):
        ...
        self._faceswap.train(
            self._model_person_data_path(self._person_a),
            self._model_person_data_path(self._person_b),
    ...
    def convert(
            ...
            width=None,
            side_by_side=False):
        # Magic incantation to not have tensorflow blow up with an out of
        # memory error.
        ...
        filter = FaceFilter(self._people[filter_person]['faces'], threshold=0.6)
        ...
        print('loading video from ', media_path)
        if not photos:
...
if __name__ == '__main__':
    ...
    faceit.add_video(
        'oliver',
        'oliver_trumpcard.mp4',
        'https://www.youtube.com/watch?v=JlxQ3IUWT0I')
    faceit.add_video(
        'oliver',
        'oliver_taxreform.mp4',
        'https://www.youtube.com/watch?v=g23w7WPSaU8')
    faceit.add_video(
        'oliver',
        'oliver_zazu.mp4',
        'https://www.youtube.com/watch?v=Y0IUPwXSQqg')
    faceit.add_video(
        'oliver',
        'oliver_pastor.mp4',
    ...
    faceit.add_video(
        'oliver',
        'oliver_lorelai.mp4',
        'https://www.youtube.com/watch?v=G1xP2f1_1Jg')
    faceit.add_video(
        'fallon',
        'fallon_mom.mp4',
        'https://www.youtube.com/watch?v=gjXrm2Q-te4')
    faceit.add_video('fallon', 'fallon_charlottesville.mp4',
                     'https://www.youtube.com/watch?v=E9TJsw67OmE')
    faceit.add_video(
        'fallon',
        'fallon_dakota.mp4',
        'https://www.youtube.com/watch?v=tPtMP_NAMz0')
    faceit.add_video(
        'fallon',
        'fallon_single.mp4',
        'https://www.youtube.com/watch?v=xfFVuXN0FSI')
    faceit.add_video('fallon', 'fallon_sesamestreet.mp4',
                     'https://www.youtube.com/watch?v=SHogg7pJI_M')
    faceit.add_video(
        'fallon',
        'fallon_emmastone.mp4',
        'https://www.youtube.com/watch?v=bLBSoC_2IY8')
    faceit.add_video(
        'fallon',
        'fallon_xfinity.mp4',
        'https://www.youtube.com/watch?v=7JwBBZRLgkM')
    faceit.add_video(
        'fallon',
        'fallon_bank.mp4',
        'https://www.youtube.com/watch?v=q-0hmYHWVgE')
